...
module.exports.execute = async (client, message, args) => {
  ...
  if (args.length === 0 || args.length === 1 && (args[0] === 'help' || args[0] === 'info')) {
    ...
    if (destroyed === 1) return await message.reply('the reminder has been removed successfully!');
    return await message.reply('all your reminders have been removed successfully!');
  }
...
function parseReminder(unparsedArgs, currentDate, message) {
  ...
  if (matchRegOne) {
    ...
    const amountToAdd = ['a', 'an'].includes(matchRegOne[2].toLowerCase()) ? 1 : parseInt(matchRegOne[2]);
    ...
    const amountToAdd = matchRegThree[2] ? parseInt(matchRegThree[2]) : 1;
...
**Please note that this message will disappear in 20 seconds.**`);
  ...
  const filter = (reaction, user) => [confirm, deny].includes(reaction.emoji.name) && user.id === message.author.id;
