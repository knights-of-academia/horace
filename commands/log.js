...
module.exports.execute = async (client, message, args) => {
    if (message.channel.id === config.channels.cotw) {
        ...
        return message.channel.send(`${prefix}log is only avaliable in <#${config.channels.cotw}>`)
    }
    ...
    if (!args || args.length === 0) {// default case: no args: return the author's record 
        ...
        return await message.channel.send(`Hey, ${message.author.username}, you have vowed in ${logs.length} challenge of the week.
            If you want the details of your vows, send ${prefix}cotw log details`);
        ...
        if (mentionedUsers.size == 0){
            return await message.channel.send(`I don't know what you means. The allowed command for cowtlog are ${prefix}cowtlog, ${prefix}cowtlog details or ${prefix} @<username>`);
        } else{// use trycatch?
